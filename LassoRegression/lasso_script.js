...
# ── 3. L1 Penalty ────────────────────────────────────\ndef l1_penalty(w, lam):\n    return lam * abs(w)\n\n# ── 4. Total Lasso Loss ──────────────────────────────\ndef lasso_loss(y_true, y_pred, w, lam):\n    return mse_loss(y_true, y_pred) + l1_penalty(w, lam)\n\n# ── 5. Gradients ─────────────────────────────────────\ndef gradients(X, y_true, y_pred, w, lam):\n    n = len(y_true)\n    dw = (2/n) * np.sum((y_pred - y_true) * X) + lam * np.sign(w)\n    db = (2/n) * np.sum(y_pred - y_true)\n    return dw, db
...
def draw_regression_line(X, w, b):
    ...
    y_plot = w * x_plot + b
    plt.plot(x_plot, y_plot, color='crimson', lw=2,
             label=f'Lasso: w={w:.4f}, b={b:.4f}')
    plt.show()
...
function buildSteps(snaps) {
    ...
    snaps.forEach((s,si)=>{
        ...
        STEPS.push({
            ...
            detail:
                ...
                `λ      = ${lam}\n` +
                `w      = ${fmt(s.w,6)}\n` +
                `|w|    = ${fmt(Math.abs(s.w),8)}\n\n` +
                `Penalty = ${lam} × ${fmt(Math.abs(s.w),8)}\n` +
                `        = ${fmt(s.penalty,8)}\n\n` +
                `Unlike L2 (w²), L1 uses |w| — constant gradient\n` +
                `can push w to exactly zero (feature elimination).`
        });

        /* Step 7 — total loss */
        STEPS.push({
            kind:'step7', lines:LINE.step7,
            title:`Step 7 — Total Lasso Loss  [${iterLabel}]`,
        ...
        STEPS.push({
            ...
            detail:
                ...
                `  L1  grad term = λ·sign(w) = ${lam}×${s.w>0?'+1':s.w<0?'-1':'0'} = ${fmt(lam*(s.w>0?1:s.w<0?-1:0),8)}\n` +
        ...
        STEPS.push({
            ...
            detail:
                ...
                `(L1 penalty pulled it toward zero — may reach exactly 0)`
        });

        /* Step 10 — draw line */
        STEPS.push({
            kind:'step10', lines:LINE.step10,
            title:`Step 10 — draw_lasso_line()  [${iterLabel}]`,
            w:s.wNext, b:s.bNext, mse:s.mse, penalty:s.penalty, total:s.total,
            curves:[...curvesAccum],
            detail:
                `draw_regression_line(X, w, b)\n\n` +
    ...
    STEPS.push({
        ...
        title:'Training complete → Evaluate (Lasso)',
        ...
        detail:
            ...
            `Compare to OLS (λ=0):\n` +
            `  OLS   w = ${fmt(olsW,6)}\n` +
            `  Lasso w = ${fmt(finalSnap.wNext,6)}\n` +
            `  |Lasso w| < |OLS w| → shrinkage applied ✓\n` +
            (Math.abs(finalSnap.wNext) < 1e-4 ? `  w ≈ 0 → feature driven to zero! (L1 sparsity)` : ``)
    });
}
...
function drawCanvas(curves, currentW, currentB) {
    ...
    X_all.forEach((x,i)=>{
        ...
        ctx.fillStyle = i<splitIdx ? '#0f172a' : '#db2777';
    ...
    ctx.fillStyle='#fbcfe8'; ctx.fillText('Lasso (current)',W-125,34);
}
...
function runLasso() {
    ...
    for(let iter=0;iter<iters;iter++){
        ...
        dw=2/n*dw+lam*(w>0?1:w<0?-1:0); db=2/n*db;
    ...
    document.getElementById('metrics').innerHTML=
        ...
        card('Lasso w',      fmt(finalSnap.wNext,4), '#7c3aed');

    show(0);
}

document.getElementById('startBtn').onclick=runLasso;
...
document.getElementById('endBtn').onclick  =()=>{if(!STEPS.length)runLasso();else show(STEPS.length-1);};
...
function loadData(rows) {
    ...
    document.getElementById('calc').innerHTML=
        `<span style="color:#475569;">${X_all.length} pts loaded. Press <span style="color:#fbcfe8;">Start</span>.</span>`;
