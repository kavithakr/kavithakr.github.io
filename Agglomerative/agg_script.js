...
function renderDistMatrix(step) {
    ...
    if (!step || !step.clusterDist) {
        box.innerHTML='<span class="text-muted small">—</span>'; return;
    }
...
function renderMergeLog(step) {
    ...
    if(!step||!step.merges||!step.merges.length){box.innerHTML='<span class="text-muted">No merges yet</span>';return;}
    box.innerHTML=step.merges.map((m,i)=>
        `<div class="mb-1"><span class="badge" style="background:${PALETTE[i%PALETTE.length]};font-size:.58rem;">
           Merge ${i+1}</span> C${m.a}+C${m.b}→C${m.newId} <span class="text-muted">(d=${fmt(m.dist,3)})</span></div>`
    ).join('');
}
...
function show(i) {
    ...
    document.getElementById('calc').innerHTML=
        `<div class="fw-bold mb-1" style="color:#1e3a5f;">${s.title}</div>`+
        `<div style="color:#334155;">${s.detail.replace(/</g,'&lt;')}</div>`;
...
function loadData(arr) {
    ...
    document.getElementById('dist-matrix-wrap').innerHTML='<span class="text-muted small">—</span>';
