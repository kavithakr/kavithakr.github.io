...
function renderTree(tree, currentId) {
    ...
    const W = 560, H = 340, padX = 45, padY = 28;
    const sx = x => padX + (maxX ? (x / maxX) * (W - 2 * padX) : (W - 2 * padX) / 2);
    const sy = d => padY + (maxD ? (d / maxD) * (H - 2 * padY - 20) : 0);
    ...
    (function drawEdges(n) {
        n.children.forEach(c => {
            const x1 = sx(n._x), y1 = sy(n._d) + 14;
            const x2 = sx(c._x), y2 = sy(c._d) - 15;
    ...
    positions.forEach(n => {
        ...
        const isLeaf = n.leaf;
        const w = isLeaf ? 50 : 62, h = 19;

        let cls;
        if (!isLeaf) cls = 'nd-dec';
        else {
            const keys = Object.keys(n.dist);
            const positive = keys.length && String(n.value).toLowerCase().match(/^(yes|1|true|y)$/);
        ...
        g.appendChild(el('rect', {
            ...
            rx: isLeaf ? 5 : 14, ry: isLeaf ? 5 : 14, class: cls
        }));

        const t1 = el('text', {
            x, y: y - 5, 'text-anchor': 'middle', 'dominant-baseline': 'central',
            class: isLeaf ? 't-leaf' : 't-dec'
        });
        t1.textContent = isLeaf ? String(n.value) : n.feature;
        g.appendChild(t1);

        const t2 = el('text', {
            x, y: y + 10, 'text-anchor': 'middle', 'dominant-baseline': 'central',
            class: isLeaf ? 't-leafsub' : 't-sub'
        });
        t2.textContent = isLeaf ? `n=${n.count}` : `g=${(n.gain ?? 0).toFixed(3)}`;
        g.appendChild(t2);
    });
}
